...
function makeTmpHome(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "sheets-auth-"));
}
...
describe("runAuth — happy path", () => {
  it("exchanges the code, writes <account>.sheets.json with both scopes, chmod 0600", async () => {
    writeClientJson(tmpHome, {
      installed: {
        client_id: "sheets-test-client.apps.googleusercontent.com",
        client_secret: "sheets-test-secret",
      },
    });
    ...
    server.use(
      http.post(TOKEN_URL, async ({ request }) => {
        ...
        return HttpResponse.json({
          access_token: "fresh-sheets-access",
          refresh_token: "rt-sheets-123",
          expires_in: 3600,
          scope: SCOPE_STRING,
          token_type: "Bearer",
        });
    ...
    expect(written).toEqual({
      token: "fresh-sheets-access",
      refresh_token: "rt-sheets-123",
      token_uri: TOKEN_URL,
      client_id: "sheets-test-client.apps.googleusercontent.com",
      client_secret: "sheets-test-secret",
      scopes: [SPREADSHEETS_SCOPE, DRIVE_SCOPE],
      expiry: "2026-06-27T11:00:00.000Z",
    });
    ...
    expect(params.get("client_id")).toBe(
      "sheets-test-client.apps.googleusercontent.com",
    );
    expect(params.get("client_secret")).toBe("sheets-test-secret");
    expect(params.get("redirect_uri")).toBe(FAKE_REDIRECT);
  });

  it("falls back to both Sheets+Drive scopes when the token response omits scope", async () => {
    writeClientJson(tmpHome, {
    ...
    expect(written.scopes).toEqual([SPREADSHEETS_SCOPE, DRIVE_SCOPE]);
    expect(written.expiry).toBe("2026-01-01T02:00:00.000Z");
  });

  it("builds an authorization URL with the two scopes space-joined", () => {
    const url = buildAuthorizationUrl("abc", FAKE_REDIRECT);
    expect(url).toContain(AUTHORIZATION_URL_BASE);
    expect(url).toContain("access_type=offline");
    expect(url).toContain("prompt=consent");
    // URLSearchParams serializes the space between the two scopes as "+"
    // (an equivalent, Google-accepted query encoding of " ").
    const scopeParam = new URL(url).searchParams.get("scope");
    expect(scopeParam).toBe(SCOPE_STRING);
    expect(url).toContain(
      `scope=${encodeURIComponent(SPREADSHEETS_SCOPE)}+${encodeURIComponent(DRIVE_SCOPE)}`,
    );
    expect(SHEETS_SCOPES).toEqual([SPREADSHEETS_SCOPE, DRIVE_SCOPE]);
  });
});
