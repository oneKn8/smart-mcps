...
| `VERCEL_SMART_DENIED_TEAMS` | comma-separated team slugs | Teams whose projects can never be mutated (any write throws before the network call; reads are unaffected). This list only ADDS to the built-in default `example-denied-team`, which cannot be removed. |

Writes resolve the project strictly: a bare project name that matches projects in more than one team throws an ambiguity error listing the candidates, so a mutation never silently hits the wrong team. Every write also passes a team deny-list check (`VERCEL_SMART_DENIED_TEAMS`, default-denies `example-denied-team`) so mutations to protected teams are refused while reads still work. Deployment-scoped ops (`get_deployment`, `deployment_logs`, `cancel_deployment`, `delete_deployment`) require a `project` param to derive the correct team scope.
