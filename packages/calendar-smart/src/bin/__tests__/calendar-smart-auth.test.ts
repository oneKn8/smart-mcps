...
import { runAuth, AUTHORIZATION_URL_BASE } from "../calendar-smart-auth.js";
...
function makeTmpHome(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "calendar-auth-"));
}
...
describe("runAuth — happy path", () => {
  it("exchanges the code, writes the token file with the expected shape, and chmods 0600", async () => {
    writeClientJson(tmpHome, {
      installed: {
        client_id: "cal-test-client.apps.googleusercontent.com",
        client_secret: "cal-test-secret",
      },
    });
    ...
    // File written at the right path with the right shape.
    const expectedPath = path.join(
      tmpHome,
      ".santo-agent",
      "oauth",
      "alpha-account.calendar.json",
    );
    ...
    expect(written).toEqual({
      token: "fresh-cal-access",
      refresh_token: "rt-cal-123",
      token_uri: TOKEN_URL,
      client_id: "cal-test-client.apps.googleusercontent.com",
      client_secret: "cal-test-secret",
      scopes: [CALENDAR_SCOPE],
      expiry: "2026-05-13T11:00:00.000Z",
    });
  ...
  it("returns the authorization URL with the right query params (used by the human prompt)", () => {
    // Sanity check: caller can build the URL itself (we expose
    ...
    expect(url.toString()).toContain("scope=" + encodeURIComponent(CALENDAR_SCOPE));
    expect(url.toString()).toContain("access_type=offline");
    expect(url.toString()).toContain("prompt=consent");
  });
});
...
describe("runAuth — error paths", () => {
  ...
  it("missing client.json throws a clear error mentioning the expected path and Console", async () => {
    // No client.json written.
    await expect(
      runAuth({
        account: "alice",
        home: tmpHome,
  ...
  it("missing client.json error names ~/.santo-agent/oauth/client.json", async () => {
    try {
      await runAuth({
        account: "alice",
        home: tmpHome,
  ...
  it("client.json without client_id / client_secret throws a clear error", async () => {
  ...
  it("token endpoint returning 4xx propagates a clear error and does NOT write a half-baked file", async () => {
    writeClientJson(tmpHome, {
      installed: {
    ...
    // No partial file should be on disk.
    const expectedPath = path.join(
      tmpHome,
      ".santo-agent",
      "oauth",
  ...
  it("token endpoint returning a malformed response (no access_token) propagates a clear error", async () => {
    writeClientJson(tmpHome, {
      installed: {
        client_id: "good-client",
        client_secret: "good-secret",
